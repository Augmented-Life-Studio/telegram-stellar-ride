 ...
 * console.log(deepLink); // Outputs: https://wallet.metapro.one/app/wc?uri=wc:example?relay-protocol=xyz
 *
 * @remarks
...
export const getMetaproDeepLink = (wcUrl: string) => {
  if (wcUrl?.includes('?expiryTimestamp=')) {
    ...
    return `https://wallet.metapro.one/app/wc?uri=${wcNew}`
  }
  return `https://wallet.metapro.one/app/wc?uri=${wcUrl}`
}
